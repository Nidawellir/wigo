...
final class SMSCodeView: UIView {
    ...
    weak var delegate: SMSCodeViewDelegate?
...
extension SMSCodeView: ConfirmationCodeInputViewDelegate {
    func didChangedText(on text: String) {
        if text.count == 6 {
            self.delegate?.textFieldDidEdeting(with: text)
        }
    }
